...
# 每个机组的流量 m3/s
flow_rate = np.array([1055, 1113, 1735, 1880, 1832, 2302])/3600
...
for i in range(T):
    electricity_price_actual[i] = electricity_price[i // 3600]
...
# 瞬时流量约束
model.addConstrs((quicksum(X[t, j] * flow_rate[j] for j in range(N)) >= flow_rate_min_m3h/3600 for t in range(T)), name="flow_rate_min_def")
model.addConstrs((quicksum(X[t, j] * flow_rate[j] for j in range(N)) <= flow_rate_max_m3h/3600 for t in range(T)), name="flow_rate_max_def")

# 切换次数约束
# model.addConstr(quicksum(S[t] for t in range(0, 9*3600)) <= max_switches_per_shift, name="max_switches_per_shift_def")
# model.addConstr(quicksum(S[t] for t in range(9*3600, 21*3600)) <= max_switches_per_shift, name="max_switches_per_shift_def")
# model.addConstr(quicksum(S[t] for t in range(21*3600, 24*3600)) <= max_switches_per_shift, name="max_switches_per_shift_def")
...
# model.addConstrs((S[0] >= X[0, j] - X[T-1, j] for j in range(N)), name="switch_logic_def")
# model.addConstrs((S[0] >= X[T-1, j] - X[0, j] for j in range(N)), name="switch_logic_def")

# 分段成本计算
model.addConstrs((opex_cost[t] == electricity_price_actual[t] / 3600 * quicksum(X[t, j] * power_rate[j] for j in range(N)) for t in range(T)), name="opex_cost_def")
...
# # 保存 S_result 到 CSV 文件
# np.savetxt("S_result.csv", S_result, delimiter=",", fmt="%d")
